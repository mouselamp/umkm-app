<x-app-layout :title="'Karyawan'" :breadcrumbs="[['label' => 'Master Data'], ['label' => 'Karyawan']]">
    <div x-data="{
        employees: [],
        pagination: null,
        loading: true,
        ...
        form: { name: '', phone: '', position: '' },

        resetForm() {
            this.form = { name: '', phone: '', position: '' };
            this.formErrors = {};
            this.selected = null;
        },

        openCreateModal() { this.resetForm(); this.modalMode = 'create'; this.showModal = true; },
        openEditModal(item) { this.resetForm(); this.modalMode = 'edit'; this.selected = item; this.form = { name: item.name, phone: item.phone || '', position: item.position || '' }; this.showModal = true; },
        openDeleteModal(item) { this.selected = item; this.showDeleteModal = true; },

        async save() {
            this.formLoading = true;
            $store.loading.start('Menyimpan karyawan...');
            this.formErrors = {};

            const url = this.modalMode === 'create' ? '/api/v1/employees' : `/api/v1/employees/${this.selected.id}`;
        ...
        async remove() {
            this.formLoading = true;
            $store.loading.start('Menghapus karyawan...');
            try {
                const response = await fetch(`/api/v1/employees/${this.selected.id}`, {
                    method: 'DELETE',
        ...
        async fetch() {
            ...
            try {
                ...
                const response = await fetch(`/api/v1/employees?${params}`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
                const data = await response.json();
        ...
        <x-page-header title="Karyawan" description="Kelola data karyawan dan informasi kontak">
            <x-slot:action>
        ...
        <div class="mb-6">
            <div class="relative max-w-md">
                ...
                <input x-model.debounce.500ms="search" type="text" class="input pl-9" placeholder="Cari karyawan...">
            </div>
        </div>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-muted/50 hover:bg-muted/50">
                        <th class="p-4 text-sm font-semibold text-foreground">Nama Karyawan</th>
                        <th class="p-4 text-sm font-semibold text-foreground">Telepon</th>
                        <th class="p-4 text-sm font-semibold text-foreground">Jabatan</th>
                ...
                <tbody class="divide-y divide-border">
                    ...
                    <template x-for="item in employees" :key="item.id">
                        <tr class="hover:bg-muted/30">
                            ...
                            <td class="p-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-info/10 text-info" x-text="item.position || '-'"></span>
                            </td>
        ...
        <div x-show="showModal" x-transition.opacity class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" style="display: none;">
            <div @click.away="showModal = false" x-show="showModal" x-transition class="bg-card rounded-xl shadow-xl w-full max-w-md">
                <div class="flex items-center justify-between p-6 border-b border-border">
                    <h3 class="text-lg font-bold text-foreground" x-text="modalMode === 'create' ? 'Tambah Karyawan' : 'Edit Karyawan'"></h3>
                ...
                <form @submit.prevent="save()" class="p-6 space-y-4">
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-foreground">Nama Karyawan</label>
                        <input type="text" x-model="form.name" class="input" placeholder="Masukkan nama karyawan" required>
                        <template x-if="formErrors.name">
                            <p class="text-sm text-destructive" x-text="formErrors.name[0]"></p>
                        </template>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-foreground">Telepon</label>
                        <input type="text" x-model="form.phone" class="input" placeholder="Masukkan nomor telepon">
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium text-foreground">Jabatan</label>
                        <input type="text" x-model="form.position" class="input" placeholder="Masukkan jabatan">
                    </div>
        ...
        <div x-show="showDeleteModal" x-transition.opacity class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" style="display: none;">
            <div @click.away="showDeleteModal = false" x-show="showDeleteModal" x-transition class="bg-card rounded-xl shadow-xl w-full max-w-md p-6">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-foreground">Hapus Karyawan?</h3>

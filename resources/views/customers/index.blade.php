<x-app-layout :title="'Pelanggan'" :breadcrumbs="[['label' => 'Master Data'], ['label' => 'Pelanggan']]">
    <div x-data="{
        customers: [],
        pagination: null,
        loading: true,
        ...
        async save() {
            ...
            $store.loading.start('Menyimpan pelanggan...');
            this.formErrors = {};

            const url = this.modalMode === 'create' ? '/api/v1/customers' : `/api/v1/customers/${this.selected.id}`;
        ...
        async remove() {
            this.formLoading = true;
            $store.loading.start('Menghapus pelanggan...');
            try {
                const response = await fetch(`/api/v1/customers/${this.selected.id}`, {
                    method: 'DELETE',
        ...
        async fetch() {
            ...
            try {
                ...
                const response = await fetch(`/api/v1/customers?${params}`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
                const data = await response.json();
        ...
        <x-page-header title="Pelanggan" description="Kelola data pelanggan dan informasi kontak">
            <x-slot:action>
        ...
        <div class="mb-6">
            <div class="relative max-w-md">
                ...
                <input x-model.debounce.500ms="search" type="text" class="input pl-9" placeholder="Cari pelanggan...">
            </div>
        </div>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-muted/50 hover:bg-muted/50">
                        <th class="p-4 text-sm font-semibold text-foreground">Nama Pelanggan</th>
                        ...
                        <th class="p-4 text-sm font-semibold text-foreground text-right">Total Pesanan</th>
                ...
                <tbody class="divide-y divide-border">
                    ...
                    <template x-for="item in customers" :key="item.id">
                        <tr class="hover:bg-muted/30">
                            ...
                            <td class="p-4 text-sm font-medium text-foreground text-right" x-text="item.orders_count || 0"></td>
        ...
        <div x-show="showModal" x-transition.opacity class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" style="display: none;">
            <div @click.away="showModal = false" x-show="showModal" x-transition class="bg-card rounded-xl shadow-xl w-full max-w-lg">
                <div class="flex items-center justify-between p-6 border-b border-border">
                    <h3 class="text-lg font-bold text-foreground" x-text="modalMode === 'create' ? 'Tambah Pelanggan' : 'Edit Pelanggan'"></h3>
                ...
                <form @submit.prevent="save()" class="p-6 space-y-4">
                    <div class="space-y-2">
                        ...
                        <input type="text" x-model="form.name" class="input" placeholder="Nama pelanggan" required>
        ...
        <div x-show="showDeleteModal" x-transition.opacity class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" style="display: none;">
            <div @click.away="showDeleteModal = false" x-show="showDeleteModal" x-transition class="bg-card rounded-xl shadow-xl w-full max-w-md p-6">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-foreground">Hapus Pelanggan?</h3>
